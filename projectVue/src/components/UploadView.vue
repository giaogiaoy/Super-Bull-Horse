...
const handleUpload = async (uploadFile: UploadFile) => {
  try {
    ...
    if (data.exists) {
      ...
      // 使用后端返回的 URL
      emit('update:modelValue', `http://localhost:3000${data.url}`)
    ...
    // 使用后端返回的 URL
    emit('update:modelValue', `http://localhost:3000${response.data.url}`)
...
<template>
  <div class="uploadView">
    <div class="uploadImg" v-if="uploadType === 'image'">
    ...
    <div class="el-upload__tip"  v-if="uploadType === 'image'">
          注意提示：
          <br />
          1.上传文件大小不能超过1GB
          <br />
    ...
    <el-upload
      ...
      :accept="accept"
    >
      <el-button type="primary" :disabled="uploadStatus.isUploading">选择文件</el-button>

      <template #tip>
        <div class="upload-status" v-show="uploadStatus.show">
          <el-progress :percentage="uploadStatus.progress" style="width: 50%;" />
    ...
    <el-dialog v-model="dialogVisible">
      <img v-if="dialogVisible"  style="width: 100%; height: 100%" :src="dialogImageUrl || modelValue" alt="Preview Image" />
    </el-dialog>
